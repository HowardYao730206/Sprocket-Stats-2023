...
import GameGrid from "../components/GameGrid"; // idk how to test this
...
function DataEntry(props) {
  ...
  // trying to do this thing but this error is AHHHHHHHHHHHH
  // const [sections, setSections] = React.useReducer(
  //   reducer,
  //   initialState
  // );
  ...
  async function generateFields() {
    ...
    await axios
      .get(
        ...
        `https://backend2022-ss-production.up.railway.app/api/v1/templates/templateId/${templateId}`,
        config
      )
      .then((res) => {
    ...
    for (var i in template.inputFields) {
      ...
      for (var j in template.inputFields[i].fields) {
        ...
        if (template.inputFields[i].fields[j].type === "Number") {
          ...
          // Game Grid
        } else if (template.inputFields[i].fields[j].type === "Grid") {
          fields.push(
            <GameGrid
              parentCallback={getInput}
              id={i + " " + j}
  ...
  function submitForm() {
    ...
    if (navigator.onLine) {
      ...
      while (offlineKeys.length > 0) {
        ...
        entry = JSON.parse(
          localStorage.getItem(offlineKeys.at(offlineKeys.length - 1))
        );
        ...
        axios
          .post(
            `https://backend2022-ss-production.up.railway.app/api/v1/datasets/datasetId/${datasetId}/entry`,
            {
    ...
    if (checkForm()) {
      ...
      if (navigator.onLine) {
        ...
        // localStorage.clear(); // works but also logs out da dude
        localStorage.removeItem(key + keyCounter); // supposed to clear local storage
      }
    } else {
